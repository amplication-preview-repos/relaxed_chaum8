/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { QuizCreateNestedManyWithoutAdminsInput } from "./QuizCreateNestedManyWithoutAdminsInput";
import { Type } from "class-transformer";
import { RoomCreateNestedManyWithoutAdminsInput } from "./RoomCreateNestedManyWithoutAdminsInput";

@InputType()
class AdminCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  password?: string | null;

  @ApiProperty({
    required: false,
    type: () => QuizCreateNestedManyWithoutAdminsInput,
  })
  @ValidateNested()
  @Type(() => QuizCreateNestedManyWithoutAdminsInput)
  @IsOptional()
  @Field(() => QuizCreateNestedManyWithoutAdminsInput, {
    nullable: true,
  })
  quizzes?: QuizCreateNestedManyWithoutAdminsInput;

  @ApiProperty({
    required: false,
    type: () => RoomCreateNestedManyWithoutAdminsInput,
  })
  @ValidateNested()
  @Type(() => RoomCreateNestedManyWithoutAdminsInput)
  @IsOptional()
  @Field(() => RoomCreateNestedManyWithoutAdminsInput, {
    nullable: true,
  })
  rooms?: RoomCreateNestedManyWithoutAdminsInput;
}

export { AdminCreateInput as AdminCreateInput };
